...
static int mazeW=width, mazeH=height;
static Cell maze[5][8];
static uint8_t dist[5][8];
...
void showWall(int x,int y,Heading h){char d='n'; if(h==E)d='e'; else if(h==S)d='s'; else if(h==W)d='w'; API::setWall(x,y,d);} 
...
bool senseAllSidesAndCheckNew() {
    bool changed = false;
    bool wf = senseRelative(N); 
    bool wr = senseRelative(E);
    //bool wb = false;
    bool wl = senseRelative(W);
...
void turnLeft() {
   /* setMotorPWM(-120, 120);
    ...
    API::turnLeft();
    facing_= Heading((int(facing_)+3)&3);
}
...
void turnRight() {
    ...
    API::turnRight();
    facing_ = Heading((int(facing_)+1) & 3);
}
/*
void stepBack() {
    setMotorPWM(-100,-100);
...
void stepForward() {
    ...
    delay(10); */
    if(API::wallFront()){
        std::cout << "Can't go fwd" << std::endl;
    }
    API::moveForward();
...
void returnToStart(){
    ...
    while(!atGoal()){
        ...
        senseAllSidesAndCheckNew(); recomputeDistances();
    }
}
...
void solve() {
    ...
    if (runFast) {
        ...
        if(!returnPath.empty()){
            std::cout << "Return Path found!" << std::endl;
        }
        if (returnPath.empty()) {
    ...
    if (runFast) {
        ...
        //delay(5000);
        std::this_thread::sleep_for(std::chrono::milliseconds(2000));        // Return to start and record return path
        returnToStart();
    }
}
